...
class Pyx(BaseEstimator, TransformerMixin):
    ...
    def __init__(
        ...
        if depth > 2:
            warnings.warn(
                "Depth too high, it will probably take really long to finish!", Warning
            )
        self.depth = depth
        self.palette = palette
    ...
    def _pyxelate(self, X):
        ...
        def _wrapper(dim):
            ...
            sum_prod = view_as_blocks((sobel * dim), (self.sobel, self.sobel)).sum(
                (2, 3)
            )
    ...
    def transform(self, X, y=None):
        ...
        if self.depth:
            ...
            new_h, new_w = new_h * (self.sobel**self.depth), new_w * (
                self.sobel**self.depth
            )
        X_ = resize(
